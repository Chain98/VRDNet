...
def load_checkpoint(checkpoint_dir, Model, name, learnrate=None):
    if os.path.exists(checkpoint_dir + name):
        ...
        if learnrate is not None:
            ...
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)  # Provide a default learnrate
        #try:
            #optimizer = torch.optim.Adam(model.parameters(), lr=learnrate)
        #except:
...
def tensor_metric(img, imclean, model, data_range=1):#计算图像PSNR输入为Tensor
    ...
    for i in range(img_cpu.shape[0]):
        if model == 'PSNR':
            ...
            #img_cpu_gray = color.rgb2gray(img_cpu[i])
            #imgclean_gray = color.rgb2gray(imgclean[i])
            #img_cpu = img.data.cpu().numpy().astype(np.float32).squeeze()
            #imgclean = imclean.data.cpu().numpy().astype(np.float32).squeeze()
            SUM += compare_ssim(imgclean[i, :, :, :], img_cpu[i, :, :, :], multichannel=True)
            #SUM += compare_ssim(imgclean_gray, img_cpu_gray, data_range=data_range, multichannel=True)
            #SUM += ssim(imgclean[i, :, :, :], img_cpu[i, :, :, :])
            #SUM += compare_ssim(imgclean, img_cpu, data_range=data_range, multichannel=True)
        else:
            print('Model False!')
...
def load_excel(x):
    ...
    writer = pd.ExcelWriter('./log/Metric.xlsx')
